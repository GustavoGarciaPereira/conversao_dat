...
Permite selecionar arquivos .dat, .sps e definir saída .csv ou .xlsx,
com opções de apply-labels, clean, no-backup, hash e Excel,
...
class Dat2CsvGUI:
    ...
    def _build_widgets(self) -> None:
        ...
        ttk.Checkbutton(
            ...
            command=self._alternar_extensao_saida,
    ...
    def _selecionar_csv(self) -> None:
        # Abre diálogo para selecionar arquivo de saída (.csv ou .xlsx)
        if self.var_excel.get():
            titulo = "Selecionar arquivo .xlsx de saída"
            ext = ".xlsx"
            tipos = [("Arquivos Excel", "*.xlsx"), ("Todos os arquivos", "*.*")]
        else:
            titulo = "Selecionar arquivo .csv de saída"
            ext = ".csv"
            tipos = [("Arquivos CSV", "*.csv"), ("Todos os arquivos", "*.*")]
    ...
    def _alternar_extensao_saida(self) -> None:
        """Troca a extensão do campo de saída entre .csv e .xlsx."""
        atual = self.csv_path.get().strip()
        if not atual:
            return
        p = Path(atual)
        if self.var_excel.get():
            if p.suffix.lower() != ".xlsx":
                self.csv_path.set(str(p.with_suffix(".xlsx")))
        else:
            if p.suffix.lower() != ".csv":
                self.csv_path.set(str(p.with_suffix(".csv")))
    ...
    def _converter(self) -> None:
        ...
        # Se o caminho de saída não foi informado, usa <input>.csv ou .xlsx
        if not self.csv_path.get().strip():
            input_p = Path(self.dat_path.get().strip())
            ext = ".xlsx" if self.var_excel.get() else ".csv"
            self.csv_path.set(str(input_p.with_suffix(ext)))
    ...
    def _executar_conversao(self) -> None:
        ...
        try:
            ...
            self._erro(f"Erro de dependência:\n{exc}")
